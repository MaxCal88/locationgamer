...
To find the shortest path, this package implements Djikstra's algorithm. To showcase the applicability of the package, we first create a network consisting of six nodes, which are randomly distributed over the x-y-plane. The edges highlighted in red constitute the shortest path between vertex one and vertex six.
...
yMin <- 0
yMax <- 100
coordMatrix <- locationgamer::randomCoordinates(nNodes,xMax,xMin,yMax,yMin)
...
djikstraResult <- locationgamer::djikstra(edgeMatrix, coordMatrix, startNode, endNode, nNodes)
locationgamer::plotDjikstra(edgeMatrix, coordMatrix, djikstraResult[[1]])
```
